...
*/
client.conf = {
  token: "NzY1MTA4MzA2MjIxODU4ODM2.X4QAow.CLF_MHnSzntomQp3BotYNt4Da8U",
  pref: "r?",
  own: "700854337026654228",
  oynuyor: "Security Bot",
  durum: "dnd"
};

client.on('userUpdate', async user => {
  let sunucuid = "655218562000224258"; //Buraya sunucunuzun IDsini yazın
  let tag = "Ж"; //Buraya tagınızı yazın
  let rol = "701051426121449502"; //Buraya tag alındığı zaman verilecek rolün IDsini yazın
  let channel = client.guilds.get(sunucuid).channels.find(x => x.name == 'ж│kingdomtag'); //tagrol-log yerine kendi log kanalınızın ismini yazabilirsiniz
  if (!tag) return;
  if (!rol) return;
  if (!channel) return;
...
client.on("ready", () => {
  client.channels.get("701047320619253780").join();
...
client.on("ready", () => {
  ...
  console.log(
    `[somon] ${client.user.username} ismi ile Discord hesabı aktifleştirildi!`
  );
